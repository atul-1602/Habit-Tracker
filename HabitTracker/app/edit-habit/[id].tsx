...
import { useHabit, useUpdateHabit } from '../../src/hooks/useHabits';
...
export default function EditHabitScreen() {
  ...
  useEffect(() => {
    if (habit) {
      ...
      setRepeatMode(habit.frequency);
      setActiveDays(habit.frequency_days || [0, 1, 2, 3, 4, 5, 6]);
      setReminderEnabled(habit.reminder_enabled || false);
      setReminderTime(habit.reminder_time || '07:00');
    }
  }, [habit]);
  ...
  const handleSave = () => {
    if (!name.trim()) return;
    if (!user?.id || !id) {
      Alert.alert('Error', 'Missing user or habit info.');
      return;
    }
    ...
    updateHabit(
      ...
      },
      {
        ...
        onError: (err: any) => Alert.alert('Error', err.message ?? 'Could not update habit.'),
      }
    );
  };
